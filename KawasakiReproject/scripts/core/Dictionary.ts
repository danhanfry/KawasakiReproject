...
class Dictionary<K, V> {
	...
	public All = (predicate: (value?: IDictionaryPair<K, V>, index?: number, list?: IDictionaryPair<K, V>[]) => boolean): boolean => {
	...
	public DefaultIfEmpty = (defaultValue?: IDictionaryPair<K, V>): Dictionary<K, V> => {
		return this.Count() ? this : new Dictionary<K, V>();
	}
	...
	public Except = (source: Dictionary<K, V>): Dictionary<K, V> => {
		return this.Where(x => !source.Contains(x));
	}

	public ForEach = (action: (value?: IDictionaryPair<K, V>, index?: number, list?: IDictionaryPair<K, V>[]) => any): void => {
		return this.ToArray().forEach(action);
	}
	...
	public IndexOf = (key: K): number => {
		...
		let index: number = -1;
		const kvArray = this.ToArray();
		for (let i = 0; i < kvArray.length; i++) {
			var currentKVPair = kvArray[i];
			if (currentKVPair.key === key) {
				index = i;
			}
	...
	public Intersect = (source: Dictionary<K, V>): Dictionary<K, V> => {
		return this.Where(x => source.Contains(x));
	}
	...
	public Remove = (key: K): boolean => {
		if (this.ContainsKey(key)) {
			return (this.RemoveAt(this.IndexOf(key)), true);
		}
	...
	public RemoveAt = (itemIndex: number): void => {
		this.ToArray().splice(itemIndex, 1);
	}
